...
public class Enemy : MonoBehaviour, IBattle, IHealth
{
    ...
    float attackCollTime = 1.0f;     // 쿨타임이 0 미만이 되면 공격
            ...
            if (state != value)
            {
                ...
                switch (state)  // 새로운 상태(새로운 상태로 들어가면서 해야 할 일 처리)
                {
                    ...
                    case EnemyState.Attack:
                        ...
                        agent.velocity = Vector3.zero;
                        anim.SetTrigger("Stop");        // 에니메이션 변경
                        attackCollTime = attackSpeed;   // 공격 쿨타임 초기화
    ...
    private void Awake()
    {
        ...
        attackArea.onPlayerIn += (target) =>
        {
            ...
            {
                attackTarget = target;
                State = EnemyState.Attack;  // 공격 삭태로 변경
            }
        };
        ...
        attackArea.onPlayerOut += (target) =>
        {
            if (attackTarget == target)
            {
                ...
                {
                    State = EnemyState.Chase;   // 플레이어가 공격범위에서 벗어나면 다시 추적 상태로
                }
            }
        };
    }
    ...
    {
        attackCollTime -= Time.deltaTime;   // 쿨타임 감소
    ...
    public void Attack(IBattle target)
    {
        ...
        attackCollTime = attackSpeed;   // 쿨타임 초기화
    }
