...
public class FollowCamera : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        target = GameManager.Inst.Player.transform;     // 대상 구하기(플레이어)
        isTargetAlive = player.IsAlive;
        player.onDie += OnTargetDie;
        offset = transform.position - target.position;  // 대상과 간격 저장해놓기
    }
    ...
    private void LateUpdate()
    {
        if (isTargetAlive)
        {
            // 카메라의 위치 -> (대상의 위치 + 간격)로 보간하며 변경. 1/moveSpeed 초에 걸쳐 목표치까지 변경
