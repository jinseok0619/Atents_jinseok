...
public class ItemData_EquipItem : ItemData, IEquipItem
{
    ...
    /// <summary>
    /// 이 아이템이 어떤 부분에 장비 될 것인지를 알려주는  프로퍼티. 상속 받는 곳에서 리턴하는 것을 변겨해야만 한다.
    /// </summary>
    ...
    public virtual void EquipItem(GameObject target, ItemSlot slot)
    {
        ...
        if(equipTarget != null)
        {
            ...
            equipTarget.EquipItem(EquipPart, slot); // ㅅarget에게 아이템 장착 시키기
        }
    }
    ...
    /// <param name="slot">아이템이 둘어있는 인벤토리 슬롯</param>
    /// <returns>장비를 햇으면 true, 해제를 했으면 false</returns>
