...
public class Player : MonoBehaviour, IFly, IDead
{
    ...
    private void Awake()
    {
        ...
        usePosition = Vector3.forward;            // 플레이어의 로컬 좌표기준으로 플레이어의 앞
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        if(collision.gameObject.CompareTag("Platform"))
        {
            ...
            platform.onMove += OnMovingObject;  // 델리게이트 켜기
        }
    }
    ...
    private void OnUseInput(InputAction.CallbackContext _)
    {
        ...
        if(colliders.Length > 0)    // 캡슐의 겹쳐진 UseableObject 컬라이더가 한개 이상이다.
        {
    ...
    {
        rigid.velocity = Vector3.zero;              // 원래 플레이어의 벨러시티 제거
        rigid.MovePosition(rigid.position + delta); // 플랫폼이 이동한 만큼 이동시키기
    }
    ...
    public void Die()
    {
        ...
        rigid.constraints = RigidbodyConstraints.None;  // 모든 회전이 간으하도록 고정해놨던 것들을 푼다.
        ...
        rigid.AddForce(transform.up * 3.0f, ForceMode.Impulse);     // 넘어질 때 약간 돌면서 넘어지게 만들기

        anim.SetTrigger("Die");     // 사망 애니메이션 실행

        onDie?.Invoke();    // 죽었을 때 다른 클래스에서 해야할 일들을 실행시키기
    }
}
