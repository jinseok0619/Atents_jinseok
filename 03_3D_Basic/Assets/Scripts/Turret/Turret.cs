...
public class Turret : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        //StartCoroutine(fireCoroutine);      // 코루틴을 자주 껐다 켰다 할 때는 코루틴을 변수에 저장하고 사용해야한다
        //StartCoroutine(PeriodFire());     // 이 코드는 PeriodFire()를 1회용으로 사용한다. 그래서 가비지가 발생된다.
    }
    ...
    IEnumerator PeriodFire()
    {
        ...
        {
            Fire();
            yield return waitFireInterval;  // 가바지룰 줄이는 방식
        }
    }
