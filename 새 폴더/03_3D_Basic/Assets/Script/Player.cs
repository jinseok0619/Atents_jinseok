...
public class Player : MonoBehaviour
{
    ...
    {
        Vector2 input = context.ReadValue<Vector2>();   // 입력된 값 읽어오기
        //Debug.Log(input);
    ...
    {
        // 리지드 바디로 회전 설정
        //rigid.MovePosition(rigid.rotation * Quaternion.Euler(0, rotateDir * rotateSpeed * Time.fixedDeltaTime, 0)); 
        rigid.MoveRotation(rigid.rotation * Quaternion.AngleAxis(rotateDir * rotateSpeed * Time.fixedDeltaTime, transform.up));

        //Quaternion.Euler(0, rotateDir * rotateSpeed * Time.fixedDeltaTime, 0)) // x,z축은 회전 값과 y축 기준으로 회전
